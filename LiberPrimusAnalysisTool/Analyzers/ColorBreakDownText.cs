...
namespace LiberPrimusAnalysisTool.Analyzers
{
    ...
    public class ColorBreakDownText
    {
        ...
        /// Handler
        /// </summary>
        public class Handler : INotificationHandler<ColorBreakDownText.Command>
        {
            /// <summary>
            /// The character repo
            /// </summary>
                ...
                Parallel.ForEach(files, file =>
                {
                    ...
                    using (var imageFromFile = new MagickImage(file))
                    {
                        ...
                        foreach (var color in colorList)
                        {
                            ...
                            contents.Add(color.ToASCIICharArray(_characterRepo));
                            contents.Add(color.ToANSICharArray(_characterRepo));
                            contents.Add("");
                        }

                        LoggingUtility.Log($"Document: {file}.round2.txt Writing file");
                        FileUtilities.WriteToFile(contents, StringUtilities.RemoveNonAlphaNum(file) + ".round2.txt");
                    }
                });
            }
        }
